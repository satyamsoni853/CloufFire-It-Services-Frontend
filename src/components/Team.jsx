...
const TeamModal = ({ member, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl w-full max-w-[600px] overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-300"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute right-6 top-6 text-gray-400 hover:text-gray-900 text-3xl transition-colors"
        >
          &times;
        </button>
        <div className="flex flex-col md:flex-row">
          ...
          <div className="flex-1 p-8 md:p-10 text-left">
            ...
            <h3 className="font-serif font-bold text-[32px] text-gray-900 leading-tight mb-4">
              {member.name}
            </h3>
            <p className="text-gray-600 font-sans text-[15px] leading-relaxed mb-8">
              {member.bio ||
                "Leading our initiatives with deep technical expertise and a passion for innovation. Committed to delivering world-class AI solutions that transform businesses and empower teams globally."}
            </p>
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary font-bold hover:bg-primary hover:text-white transition-colors cursor-pointer">
                in
              </div>
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary font-bold hover:bg-primary hover:text-white transition-colors cursor-pointer">
                f
