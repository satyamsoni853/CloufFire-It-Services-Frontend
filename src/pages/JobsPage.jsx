...
const JobsPage = () => {
  ...
  const uniqueIndustries = useMemo(
    () => [...new Set(jobs.map((j) => j.industry_type).filter(Boolean))],
    [jobs],
  );
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      const matchesMode =
        ...
        filters.work_mode.includes(job.work_mode);
      ...
      const matchesIndustry =
        !filters.industry || job.industry_type === filters.industry;
      // Experience filter
      let matchesExp = true;
      if (filters.experience.length > 0) {
        ...
        matchesExp = filters.experience.some((f) => {
          if (f === "Fresher")
            return exp.includes("fresher") || expNumbers.includes(0);
          if (f === "1-3 years")
            return expNumbers.some((n) => n >= 1 && n <= 3);
          if (f === "3-5 years")
            return expNumbers.some((n) => n >= 3 && n <= 5);
            ...
            <div
              className={`w-4.5 h-4.5 rounded-md border-2 flex items-center justify-center transition-all ${
                filters[filterKey].includes(opt)
                  ? "bg-[#ff7301] border-[#ff7301]"
                  : "border-gray-300 group-hover:border-[#ff7301]"
              }`}
            >
              {filters[filterKey].includes(opt) && (
                <svg
  ...
  return (
    <>
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 font-medium">
            {role === "jobseeker"
              ...
              : "Manage and track all posted job opportunities."}
          </p>
        </div>
        {(role === "employer" || role === "admin") && (
          <a
            href="/dashboard/jobs/post"
            className="bg-[#ff7301] text-white px-6 py-3.5 rounded-[20px] font-semibold shadow-lg shadow-orange-100 hover:bg-orange-600 transition-all flex items-center cursor-pointer shrink-0 text-sm"
          >
            <svg
      ...
      {/* Search Bar */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-8 flex flex-col sm:flex-row gap-3">
        ...
        >
          ...
          Filters{" "}
          {activeFilterCount > 0 && (
            <span className="bg-white/20 text-white px-2 py-0.5 rounded-full text-[9px] font-bold">
              {activeFilterCount}
            </span>
          )}
        </button>
        {activeFilterCount > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-5 py-3.5 rounded-[20px] text-rose-500 hover:bg-rose-50 font-semibold text-xs transition-all cursor-pointer"
          >
            <X size={14} /> Clear
          </button>
        )}
      ...
      <div className="flex items-center justify-between mb-6 px-2">
        <p className="text-xs text-gray-500 font-medium tracking-wide">
          ...
          of <span className="text-gray-900 font-semibold">{jobs.length}</span>{" "}
          results
        </p>
      </div>

      <div
        className={`grid gap-10 ${showFilters ? "grid-cols-1 lg:grid-cols-12" : "grid-cols-1"}`}
      >
        {/* Sidebar Filters */}
        {showFilters && (
          <div className="lg:col-span-3">
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 sticky top-32 max-h-[70vh] overflow-y-auto no-scrollbar">
              ...
              {uniqueLocations.length > 1 && (
                <div className="mb-6">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-gray-100 text-xs font-medium outline-none focus:border-[#ff7301] transition-all bg-gray-50 appearance-none"
                  >
        ...
        <div className={showFilters ? "lg:col-span-9" : ""}>
          {loading ? (
            <GlobalLoader
              variant="compact"
              message="Loading Cloudfire jobs..."
            />
            ...
            <div className="space-y-6">
              {filteredJobs.map((job) => (
                <div
                  key={job.id}
                  className={`bg-white rounded-xl border shadow-sm transition-all overflow-hidden ${expandedJob === job.id ? "border-[#ff7301]/20 shadow-md" : "border-gray-100 hover:border-gray-200"}`}
                >
                  {/* Job Card Header */}
                  <div
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                      <div className="flex items-start gap-6 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-lg text-gray-900 group-hover:text-[#ff7301] transition-colors">
                            {job.title}
                          </h3>
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 mt-2 font-medium">
                            ...
                            {job.salary && (
                              <span className="flex items-center gap-1.5 text-[#ff7301] font-semibold">
                                <DollarSign size={12} />
                                {job.salary}
                              </span>
                            )}
                          ...
                          <div className="flex flex-wrap gap-2 mt-5">
                            {job.type && (
                              <span className="px-3 py-1.5 bg-gray-50 text-gray-600 rounded-xl text-[9px] font-semibold uppercase tracking-widest border border-gray-100">
                                {job.type}
                              </span>
                            )}
                            {job.work_mode && (
                            ...
                            {job.skills_required
                              ...
                              .map((s) => (
                                <span
                                  key={s}
                                  className="px-3 py-1.5 bg-gray-50 text-[#ff7301] rounded-xl text-[9px] font-semibold uppercase tracking-widest border border-orange-50"
                                >
                                  {s.trim()}
                                </span>
                              ))}
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        {role === "jobseeker" && (
                          <>
                            <button
                              ...
                              className="p-3 text-gray-400 hover:text-[#ff7301] hover:bg-gray-50 rounded-[18px] transition-all cursor-pointer border border-gray-100"
                              title="Save Job"
                            >
                              <Bookmark size={18} />
                            </button>
                        ...
                        {(role === "admin" ||
                          (role === "employer" && job.posted_by_id)) && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDeleteJob(job.id);
                            }}
                            className="p-3 text-rose-500 hover:bg-rose-50 rounded-[18px] transition-colors cursor-pointer border border-transparent hover:border-rose-100"
                            title="Remove Job"
                          >
                            <svg
                              className="w-5 h-5"
                      ...
                      {job.company_website && (
                        <div className="mt-6 flex justify-end">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-xs text-[#ff7301] font-semibold hover:underline"
                          >
                            Visit Website →
                          </a>
                        </div>
          ...
          <div className="bg-white rounded-[40px] w-full max-w-lg overflow-hidden shadow-2xl border border-gray-100">
            <div className="p-8 border-b border-gray-50 bg-gray-50">
              <h2 className="text-xl font-semibold text-gray-900">
                Apply for {applyModal.job?.title}
              </h2>
            ...
            <div className="p-10">
              <div className="bg-orange-50/50 border border-orange-100 rounded-xl p-5 mb-8">
                <p className="text-xs text-orange-700 font-medium leading-relaxed">
                  Your professional profile and resume will be shared with the
              ...
              <textarea
                ...
                className="w-full px-5 py-4 rounded-xl border border-gray-100 focus:border-[#ff7301] outline-none transition-all resize-none bg-gray-50 text-sm font-medium"
                placeholder="Briefly explain your interest in this role..."
              ></textarea>
              <div className="mt-10 flex justify-end gap-3">
                <button
                  onClick={() => setApplyModal({ open: false, job: null })}
                  className="px-6 py-3.5 rounded-[20px] font-semibold text-gray-500 hover:bg-gray-50 transition-all cursor-pointer text-sm"
                >
                  Discard
                </button>
                <button
